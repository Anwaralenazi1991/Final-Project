...
class MindOiksDBOpenHelper(
    ...
    override fun onCreate(db: SQLiteDatabase) {
        val sql =
            ("CREATE TABLE $TABLE_NAME($COLUMN_ID INTEGER PRIMARY KEY,$COLUMN_NAME TEXT,$COLUMN_PRICE TEXT,$COLUMN_PROD_ID INTEGER)")
        db.execSQL(sql)
        val sql2 =
        ...
        db.execSQL(sql2)
        val sql3 =
            ("CREATE TABLE $REST_TABLE_NAME($COLUMN_ID INTEGER PRIMARY KEY AUTOINCREMENT,$REST_NAME TEXT,$REST_DES TEXT,$REST_LOGO TEXT, $COLUMN_DELETED INTEGER)")
        db.execSQL(sql3)

        val sql4 =
            ("CREATE TABLE $ITEM_TABLE_NAME($COLUMN_ID INTEGER PRIMARY KEY AUTOINCREMENT,$ITEM_NAME TEXT,$ITEM_PRICE TEXT,$ITEM_PHOTO TEXT,$ITEM_REST_ID INTEGER, $COLUMN_DELETED INTEGER)")
        db.execSQL(sql4)
    ...
    fun addResturant(item: Model) {
        ...
        values.put(COLUMN_DELETED, item.deleted)

        val db = this.writableDatabase

        val id =
            db.insertWithOnConflict(REST_TABLE_NAME, null, values, SQLiteDatabase.CONFLICT_IGNORE)

        if (id.toInt() == -1) {
            db.update(REST_TABLE_NAME, values, "$COLUMN_ID=?", arrayOf("${item.id}"))
        }
//        val idd = db.insert(REST_TABLE_NAME, null, values)
    ...
    fun addItemForResturant(item: MenuModel, id: Long) {
        ...
        var id =
            db4.insertWithOnConflict(ITEM_TABLE_NAME, null, values, SQLiteDatabase.CONFLICT_IGNORE)

        if (id.toInt() == -1) {
            db4.update(ITEM_TABLE_NAME, values, "$COLUMN_ID=?", arrayOf("${item.id}"))
            id = item.id.toLong()
        }
...
//                            cur.moveToNext()
                    ...
                    val deleted = it.getInt(it.getColumnIndex(COLUMN_DELETED))

                    if(deleted == 0 || deleted == null) {
                        list.add(MenuModel(name, price, photo, id))
                    }
//                        }
//                    }
                }
    ...
    fun addItem(item: Items) {
        ...
        values.put(COLUMN_PROD_ID, item.id)
    ...
    fun getAllResturants(): Cursor? {
        val db = this.readableDatabase
        return db.rawQuery("SELECT * FROM $REST_TABLE_NAME WHERE $COLUMN_DELETED != 1", null)
    }
    ...
    companion object {
        ...
        const val COLUMN_PROD_ID = "item_prod_id"
